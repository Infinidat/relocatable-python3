...
int main(int argc, char *argv[])
{
    ...
    char python_bin_path[PATH_MAX] = { 0 };
    char python_lib_path[PATH_MAX] = { 0 };
    char *base_path = NULL;
    char *bin_path = NULL;
    int len = 0;
    ...
    len = snprintf(python_bin_path, sizeof(python_bin_path),
                   "%s.bin", run_path);
    if (len > PATH_MAX) {
        return EXIT_FAILURE;
    }
    ...
    len = snprintf(python_lib_path, sizeof(python_lib_path),
                   "%s/lib", base_path);
    if (len > PATH_MAX) {
        return EXIT_FAILURE;
    }
    setenv("LIBPATH", python_lib_path, TRUE);
    if (execve(python_bin_path, argv, environ) == -1) {
        return EXIT_FAILURE;
    }
    return EXIT_SUCCESS;
}
