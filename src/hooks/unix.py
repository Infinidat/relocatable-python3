...
def run(args, env, cwd=None, verbose=True):
...
def libiconv(options, buildout, env):
    ...
    target = env.get('LibIconvTarget')
    suffix = os.path.join(target, 'x64', 'Release')
    pairs = (
        (
            os.path.join(suffix, 'iconv.dll'),
            os.path.join(prefix, 'bin')
        ),
        (
            os.path.join(suffix, 'iconv.lib'),
...
def gettext(options, buildout, env):
    cwd='libtextstyle'
    args = AUTORECONF
    run(args=args, env=env, cwd=cwd)

def autogen(options, buildout, env):
    shell = env.get('SHELL')
    args = [shell, '-exu', AUTOGEN]
    run(args=args, env=env)
...
def create_python_wrapper(options, buildout, env):
    ...
    args = compiler.split() + pflags.split() + cflags.split() + lflags.split()
    args += ['-s', hook, '-o', src]
...
def create_ncurses_fallbacks(options, buildout, env):
    ...
    args = ['ncurses/tinfo/MKfallback.sh', terminfo, src, tic, infocmp]
    args += terminals.split(',')
    data = run(args=args, env=env, verbose=False)
